    ...
    const updateTodo = () => {
        ...
        if (todos) {
            ...
            if (todosJson.filter(value => { return value.title == title }).length > 0) {
                ...
                alert("TO-DO Has Been UPDATED")
            }
            else {
                alert("TO-DO Does not Exists")
            }
        }
        else
            ...
            <section className="text-gray-600 body-font">
                <div className="container px-5 py-24 mx-auto flex flex-wrap items-center">
                    <div className="bg-gray-100 rounded-lg p-8 flex flex-col md:ml-auto w-full mt-10 md:mt-0">
                        <h2 className="text-gray-900 text-lg font-medium title-font mb-5">Update a TO-DO</h2>
                        ...
                        <button onClick={updateTodo} className="text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none w-fit hover:bg-indigo-600 rounded text-lg">Update To-Do</button>
                        <p className="text-xs text-gray-500 mt-3">Best List!</p>
                    </div>
                </div>
            </section>
        </div>
